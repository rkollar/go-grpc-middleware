/*
`grpc_zerolog` is a gRPC logging middleware backed by ZAP loggers

It accepts a user-configured `zerolog.Logger` that will be used for logging completed gRPC calls. The same `zerolog.Logger` will
...
to the ctx so that it will be present on subsequent use of the `ctx_zerolog` logger.
